...
connection = pymysql.connect(
    host='localhost', user='root', password='new_password', db='sra')
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            print(ResumeParser(save_image_path).get_extracted_data(
            ), "ResumeParser(save_image_path).get_extracted_data()  -            ------ -- - -- - - ")
        ...
        if st.button('Login'):
            if ad_user == 'user' and ad_password == 'user123':
                st.success("Welcome Milan")
                # Display Data
                ...
                # Generate pie chart for predicted field recommendations
                st.subheader(
                ...
                # Generate pie chart for User's Experienced Level
